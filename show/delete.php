...
if (strcmp($type, "student") == 0) {
    ...
    $sql = "DELETE FROM ElectiveCourse WHERE startDate='$id' AND courseCode='$course'";
    if ($conn->query($sql) === TRUE) {
        $conn->close();
        header("Refresh: 1;URL=courseinfo.php?id=$course");
    }
} else if (strcmp($type, "mandatory") == 0) {
    $sql = "DELETE FROM MandatoryCourse WHERE startDate='$id' AND courseCode='$course'";
    if ($conn->query($sql) === TRUE) {
        $conn->close();
        header("Refresh: 1;URL=courseinfo.php?id=$course");
    }
